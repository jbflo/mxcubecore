...
class DoseEstimate:
    ...
    def init(self, fname="/opt/pxsoft/bin/flux2dose", transmission=None):
       self.dose_rate = self.get_current_flux_dose(fname, transmission)
       self.fname = fname
    
    def get_current_flux_dose(fname, transmission):
        ...
        try:
            ...
            self.dose_rate = float(dose_rate_str.split()[2])
        except Exception:
    ...
    def flux_dose_estimate(self, transmission, exposure_time,  oscillation_range, nb_images):
        """ Estimate flux dose some the current data collection parameter
        Args:
            transmission (float)
            exposure_time (float)
